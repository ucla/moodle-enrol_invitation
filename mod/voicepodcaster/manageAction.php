...
$urlModuleForm=$CFG->wwwroot.'/course/mod.php?section=0&sesskey='.sesskey().
                '&id='.$session->getCourseId().'&add=voicepodcaster&rid=';
...
if ( $session->error === false && $session != NULL ) 
	...
	if ($params['action'] == 'launch') 
	{
		...
		if ($result != NULL) 
		{
		    wimba_add_log(WIMBA_DEBUG,voicepodcaster_LOGS,"launch the ". $params["type"] . ", nid =" .$result->getNid()); 
		...
		if ($params['type'] == "board") 
		{
			...
			if ($result != NULL && $result->error != "error") 
    		{
    			if($params['grade'] == 'true'){         
    			   $params['gradeid']=voicepodcaster_add_grade_column($result->getRid(),$params["enc_course_id"],$result->getTitle(),$params["points_possible"]);
    			}else{
    			  $resourceDb=$DB->get_record("voicepodcaster_resources",array("rid" => $result->getRid()));
    			  if($resourceDb->gradeid != -1 ){
        			   voicepodcaster_delete_grade_column($result->getRid(),$params["enc_course_id"]);
