...
class restore_voiceboard_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function process_voiceboard($data) {
        ...
        if($userinfo)
        {
            ...
            $resourceDbMatched = $DB->get_record("voiceboard_resources", array("fromrid" => $old_rid, "course" => $this->get_courseid(), "copyoptions" => $typeCopy)); //resource which match the current copy options
            $resourceDbOther = $DB->get_record("voiceboard_resources", array("fromrid" => $old_rid, "course" => $this->get_courseid(), "copyoptions" => "2"));
        }
        else
        {
            $copyOptions = "2";//delete all
            $resourceDbMatched = $DB->get_record("voiceboard_resources", array("fromrid" => $old_rid, "course" => $this->get_courseid(), "copyoptions" => "2"));
            $resourceDbOther = $DB->get_record("voiceboard_resources", array("fromrid" => $old_rid, "course" => $this->get_courseid(), "copyoptions" => $typeCopy));
        }
        ...
        if($isGradable == 'true') { //the old vb was gradable
            ...
            if(isset($actionGradebook) && $actionGradebook == "update") {
                ...
                voiceboard_add_grade_column($newResource->getRid(), $this->get_courseid(), $newResource->getTitle(), $newResourceCopyOptions->getPointsPossible());
            }
