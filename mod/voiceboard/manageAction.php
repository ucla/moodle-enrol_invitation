...
$urlModuleForm=$CFG->wwwroot.'/course/mod.php?section=0&sesskey='.sesskey().
                '&id='.$session->getCourseId().'&add=voiceboard&rid=';
		...
		if ($params['type'] == "board") 
		{
			...
			if ($result != NULL && $result->error != "error") 
    		{
    			if($params['grade'] == 'true'){         
    			   $params['gradeid']=voiceboard_add_grade_column($result->getRid(),$params["enc_course_id"],$result->getTitle(),$params["points_possible"]);
    			}else{
    			  $resourceDb=$DB->get_record("voiceboard_resources",array("rid" => $result->getRid()));
    			  if($resourceDb->gradeid != -1 ){
        			   voiceboard_delete_grade_column($result->getRid(),$params["enc_course_id"]);
