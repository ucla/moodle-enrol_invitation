  ...
  ********/
   $config->module="voicetools";
    if(empty($config->servername))
    {
       ...
       print_error(get_string('wrongconfigurationURLunavailable', 'voiceboard'), $_SERVER["HTTP_REFERER"]);
    }
    
    if(empty($config->adminusername))
    {
        wimba_add_log(WIMBA_ERROR,voiceboard_LOGS,get_string('emptyAdminUsername', 'voiceboard'));
        print_error(get_string('emptyAdminUsername', 'voiceboard'), $_SERVER["HTTP_REFERER"]);
    }  
    
    if(empty($config->adminpassword))
    {
        wimba_add_log(WIMBA_ERROR,voiceboard_LOGS,get_string('emptyAdminPassword', 'voiceboard'));
        print_error(get_string('emptyAdminPassword', 'voiceboard'), $_SERVER["HTTP_REFERER"]);
    } 
    ...
    if ($config->servername{strlen($config->servername)-1} == '/')
    {
        ...
        print_error(get_String('trailingSlash', 'voiceboard'), $_SERVER["HTTP_REFERER"]);
    }
    ...
    if (!preg_match('/^http:\/\//', $config->servername) && !preg_match('/^https:\/\//', $config->servername)) 
    {
        ...
        print_error(get_String('trailingHttp', 'voiceboard'), $_SERVER["HTTP_REFERER"]);
    }  
    ...
    if ($result != "ok") 
    {
        ...
        {//the error description is not in the bundle
            ...
            print_error(get_string($result, 'voiceboard' ), 'javascript:history.back();');
        }
    }
