<?php
class Elluminate_WS_SAS_Response_SessionResponse implements Elluminate_WS_APIResponseHandler{
    
   private $logger;

   public function __construct(){
      $this->logger = Elluminate_Logger_Factory::getLogger("Elluminate_WS_SAS_Response_SessionResponse");
   }
   ...
   private function loadSessionObject($apiResult){
      ...
      $sessionResponse = $apiResult->SessionResponse;
      $Session->meetingid = $sessionResponse->sessionId;
      $Session->sessionname = $sessionResponse->sessionName;
      $Session->timestart = Elluminate_WS_Utils::convertSASDateToPHPDate($sessionResponse->startTime);
      $Session->timeend = Elluminate_WS_Utils::convertSASDateToPHPDate($sessionResponse->endTime);
