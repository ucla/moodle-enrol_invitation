 ...
 */
	...
	if($elluminate->sessiontype == 2 || $elluminate->sessiontype == 3) {
		if($elluminate->groupparentid > 0) {
			$sql = "SELECT e.*
	                FROM {elluminate} e
	                WHERE e.groupparentid = :groupparentid
	                AND e.groupid = :currentgroupid";
	        $sql_params = array('groupparentid'=>$elluminate->groupparentid, 'currentgroupid'=>$currentgroupid);
			$elluminate = $DB->get_record_sql($sql, $sql_params);			
		} else {
			if($elluminate->groupmode != 0) {
				$sql = "SELECT e.*
	                FROM {elluminate} e
	                WHERE e.groupparentid = :groupparentid
	                AND e.groupid = :currentgroupid";
		        $sql_params = array('groupparentid'=>$elluminate->id, 'currentgroupid'=>$currentgroupid);
				$elluminate = $DB->get_record_sql($sql, $sql_params);
			}
		}		
		if(!$elluminate) {
...
/// Print the page header
	...
	$buttontext = "";
	if($participant) {
    	$buttontext = elluminate_update_module_button($cm->id, $course->id, $strelluminate);
	}
...
/// Handle a request to delete a recording.
    if (!empty($delrecording) &&
        ...
        if (optional_param('confirm', '', PARAM_ALPHANUM) == $sesskey) {
            ...
            echo $OUTPUT->confirm(get_string('deleterecordingconfirm', 'elluminate', userdate($recording->created)),
...
/// Unhide a recording.
    if (!empty($showrecording) && ($canmanageanyrecordings || $canmanagerecordings)) {
        if ($recording = $DB->get_record('elluminate_recordings', array('id'=>$showrecording))) {
            $recording->visible = 1;
...
/// Make a recording visible to only group members.
    if (!empty($hidegrouprecording) && ($groupmode == VISIBLEGROUPS) &&
        ...
        if ($recording = $DB->get_record('elluminate_recordings', array('id'=>$hidegrouprecording))) {
...
/// Make a recording visible to only group members.
    if (!empty($showgrouprecording) && ($groupmode == VISIBLEGROUPS) &&
        ...
        if ($recording = $DB->get_record('elluminate_recordings', array('id'=>$showgrouprecording))) {
    ...
    $formelements = array(
        ...
        get_string('sessionnamedisplay', 'elluminate') => $elluminate->sessionname,
    ...
    if ($participant && $canmanagepreloads && !$hasfinished) {
        ...
        if ($haspreload) {
                $link .= ' <a href="' . $CFG->wwwroot . '/mod/elluminate/preload.php?id=' . $elluminate->id .
                         ...
                         $CFG->wwwroot . '/mod/elluminate/pix/delete.gif" width="11" height="11" alt="' . $tooltip . '" /></a>';
...
/// Display a link to play the recording if one exists.
    if ($participant && $canviewrecordings &&
        ($recordings = $DB->get_records('elluminate_recordings', array('meetingid'=>$elluminate->meetingid), 'created ASC'))) {
        ...
        /// Include the recording description, if not empty.
            ...
            /// Display an icon to allow editing the extra description field for this recording.
				...
				if($elluminate->groupmode > 0) {
	                $link .= ' <a href="' . $CFG->wwwroot . '/mod/elluminate/view.php?id= ' . $cm->id .
	                         ...
	                         '"><img src="' . $CFG->wwwroot . '/mod/elluminate/pix/edit.gif" width="11" height="11" alt="' .
	                         $tooltip .'" /></a>';
				} else {
					$link .= ' <a href="' . $CFG->wwwroot . '/mod/elluminate/view.php?id= ' . $cm->id .
            ...
            if ($candeleteanyrecordings || ($candeleterecordings && ($elluminate->creator == $USER->id))) {
                ...
                $link .= ' <a href="' . $CFG->wwwroot . '/mod/elluminate/view.php?id=' . $cm->id .
                         ...
                         $CFG->wwwroot . '/mod/elluminate/pix/delete.gif" width="11" height="11" alt="' . $tooltip .
                         '"></a>';
            }
            ...
            /// Display an icon to change the overall recording visibility.
                if ($recording->visible) {
                    ...
                    $link .= ' <a href="' . $CFG->wwwroot . '/mod/elluminate/view.php?id=' . $cm->id .
                             ...
                             $CFG->wwwroot . '/mod/elluminate/pix/hide.gif" width="11" ' . 'height="11" alt="' . $tooltip .
                             '"></a>';
                } else {
                    ...
                    $link .= ' <a href="' . $CFG->wwwroot . '/mod/elluminate/view.php?id=' . $cm->id .
                             ...
                             $CFG->wwwroot . '/mod/elluminate/pix/show.gif" width="11" height="11" alt="' . $tooltip .
                             '"></a>';
                }
        	}
