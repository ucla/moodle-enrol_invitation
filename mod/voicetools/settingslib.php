...
class admin_setting_voicetools_voiceversion extends admin_setting {
/**
 ...
 */
    ...
    public function output_html($data, $query='') {
        ...
        return format_admin_setting($this, $this->visiblename,
            '<div class="form-text defaultsnext">'.$data.'</div>',
            $this->description, true, '', null, $query);
    }
}

class admin_setting_voicetools_integrationversion extends admin_setting {
/**
 * not a setting, just text
 * @param string $name unique ascii name, either 'mysetting' for settings that in config, or 'myplugin/mysetting' for ones in config_plugins.
 * @param string $heading heading
...
class admin_setting_voicetools_loglevel extends admin_setting_configselect {
    ...
    public function output_html($data, $query='') {
        ...
        $return .= '<a href="'.$CFG->wwwroot.'/mod/voicetools/logs.php?action=list">';
        $return .= get_string('viewlogs', 'voicetools').'</a>';
...
class admin_setting_voicetools_configtest extends admin_setting {
/**
 * not a setting, just text
 * @param string $name unique ascii name, either 'mysetting' for settings that in config, or 'myplugin/mysetting' for ones in config_plugins.
 * @param string $heading heading
 ...
 */
    ...
    public function output_html($data, $query='') {
        ...
        $return .= '<div class="form-setting"><input type="button" onclick="CheckConfiguration();" value="Check Configuration" /></div>';
        $return .= '</div>';
