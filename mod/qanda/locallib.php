...
/**
 * class to handle exporting an entire qanda database
 */
class qanda_full_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        global $DB;
        if (!$this->cm = get_coursemodule_from_id('qanda', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'qanda');
        }
        if (!$this->qanda = $DB->get_record('qanda', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'qanda');
        }
        $entries = $DB->get_records('qanda_entries', array('qandaid' => $this->qanda->id));
        list($where, $params) = $DB->get_in_or_equal(array_keys($entries));

        $aliases = $DB->get_records_select('qanda_alias', 'entryid ' . $where, $params);
        $categoryentries = $DB->get_records_sql('SELECT ec.entryid, c.name FROM {qanda_entries_categories} ec
            JOIN {qanda_categories} c
        ...
        foreach (array_keys($entries) as $entry) {
            $this->keyedfiles[$entry] = array_merge(
                    $fs->get_area_files($context->id, 'mod_qanda', 'attachment', $entry, "timemodified", false), $fs->get_area_files($context->id, 'mod_qanda', 'question', $entry, "timemodified", false), $fs->get_area_files($context->id, 'mod_qanda', 'answer', $entry, "timemodified", false), $fs->get_area_files($context->id, 'mod_qanda', 'entry', $entry, "timemodified", false)
            );
    ...
    public function prepare_package() {
        ...
        if ($this->get('exporter')->get('formatclass') == PORTFOLIO_FORMAT_SPREADSHEET) {
            $csv = qanda_generate_export_csv($entries, $aliases, $categories);
            ...
            foreach ($entries as $e) {
                $content = qanda_entry_portfolio_caller::entry_content(
                                $this->course, $this->cm, $this->qanda, $e, (array_key_exists($e->id, $aliases) ? $aliases[$e->id] : array()), $format
                );
                $entry = new portfolio_format_leap2a_entry('qandaentry' . $e->id, $e->question, 'entry', $content);
                ...
                if (!empty($this->keyedfiles[$e->id])) {
                    $writer->link_files($entry, $this->keyedfiles[$e->id], 'qandaentry' . $e->id . 'file');
            ...
            $selection = new portfolio_format_leap2a_entry('wholeqanda' . $this->qanda->id, get_string('modulename', 'qanda'), 'selection');
    ...
    public function check_permissions() {
        return has_capability('mod/qanda:export', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'qanda');
    }
...
/**
 * class to export a single qanda entry
 *
 * @package   mod-qanda
 * @copyright 1999 onwards Martin Dougiamas  {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qanda_entry_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        ...
        if ($this->entryid) {
            if (!$this->entry = $DB->get_record('qanda_entries', array('id' => $this->entryid))) {
                throw new portfolio_caller_exception('noentry', 'qanda');
            }
            // in case we don't have USER this will make the entry be printed
            $this->entry->approved = true;
        }
        $this->categories = $DB->get_records_sql('SELECT ec.entryid, c.name FROM {qanda_entries_categories} ec
            JOIN {qanda_categories} c
            ON c.id = ec.categoryid
            WHERE ec.entryid = ?', array($this->entryid));
        $context = context_module::instance($this->cm->id);
        if ($this->entry->sourceqandaid == $this->cm->instance) {
            if ($maincm = get_coursemodule_from_instance('qanda', $this->entry->qandaid)) {
                $context = context_module::instance($maincm->id);
            }
        }
        $this->aliases = $DB->get_record('qanda_alias', array('entryid' => $this->entryid));
        $fs = get_file_storage();
        $this->multifiles = array_merge(
                $fs->get_area_files($context->id, 'mod_qanda', 'attachment', $this->entry->id, "timemodified", false), $fs->get_area_files($context->id, 'mod_qanda', 'question', $entry, "timemodified", false), $fs->get_area_files($context->id, 'mod_qanda', 'answer', $entry, "timemodified", false), $fs->get_area_files($context->id, 'mod_qanda', 'entry', $this->entry->id, "timemodified", false)
        );
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cm->id);
        return has_capability('mod/qanda:exportentry', $context)
                || ($this->entry->userid == $this->user->id && has_capability('mod/qanda:exportownentry', $context));
    }
    ...
    public function prepare_package() {
        ...
        $format = $this->exporter->get('format');
        $content = self::entry_content($this->course, $this->cm, $this->qanda, $this->entry, $this->aliases, $format);

        if ($this->exporter->get('formatclass') === PORTFOLIO_FORMAT_PLAINHTML) {
            $filename = clean_filename($this->entry->question) . '.html';
            ...
            $entry = new portfolio_format_leap2a_entry('qandaentry' . $this->entry->id, $this->entry->question, 'entry', $content);
            ...
            throw new portfolio_caller_exception('unexpected_format_class', 'qanda');
        }
    }
     ...
     * for both this class and the full qanda exporter
     ...
     * @param stdclass $cm
     * @param stdclass $qanda
     * @param stdclass $entry
     *
     * @return string
     */
    public static function entry_content($course, $cm, $qanda, $entry, $aliases, $format) {
        global $OUTPUT, $DB;
        $entry = clone $entry;
        ...
        $output = '<table class="qanda-post dictionary" cellspacing="0">' . "\n";
        $output .= '<tr valign="top">' . "\n";
        $output .= '<td class="entry">' . "\n";

        $output .= '<div class="question">';
        $output .= format_text($OUTPUT->heading($entry->question, 3), FORMAT_MOODLE, $options);
        $output .= '</div> ' . "\n";

        $entry->answer = format_text($entry->answer, $entry->answerformat, $options);
        $output .= portfolio_rewrite_pluginfile_urls($entry->answer, $context->id, 'mod_qanda', 'entry', $entry->id, $format);
        ...
        if (!empty($aliases)) {
            ...
            $output .= '<tr valign="top"><td class="entry-lower-section">';
            $key = (count($aliases) == 1) ? 'alias' : 'aliases';
            $output .= get_string($key, 'qanda') . ': ';
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qanda_file_info_container extends file_info {
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        global $DB;
        $sql = 'SELECT DISTINCT f.itemid, ge.question
                  FROM {files} f
                  JOIN {modules} m ON (m.name = :modulename AND m.visible = 1)
                  JOIN {course_modules} cm ON (cm.module = m.id AND cm.id = :instanceid)
                  JOIN {qanda} g ON g.id = cm.instance
                  JOIN {qanda_entries} ge ON (ge.qandaid = g.id AND ge.id = f.itemid)
        ...
        if ($countonly !== false) {
            $sql .= ' ORDER BY ge.question, f.itemid';
        }

        $rs = $DB->get_recordset_sql($sql, $params);
        $children = array();
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_qanda', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
