    ...
    */    
    function getVTPrincipalView($message,$type=null) {
        ...
        $this->xml->addButtonElement(
                        ...
                        "javascript:LaunchElement('manageAction.php','voiceemail');");
        ...
        if($this->session->isInstructor())
        {
            $this->xml->addButtonElement(
                            ...
                            "doOpenAddActivity('../../course/mod.php','section=0&sesskey=" . sesskey() . "&add=voiceemail')");
        ...
        {//problem to get the vt resource
            ...
            $this->xml->setError(get_string("error_connection_vt", "voiceemail"));
        }
...
* ***********************************/
    ...
    */ 
    function createVPInfoPanel() {
        ...
        $this->xml->addInputElement($parameters);
        $this->xml->addSimpleLineElement("label", get_string('comment_slide', 'voiceemail'));
        ...
        $this->xml->addInputElement($parameters);
        $this->xml->addSimpleLineElement("label", get_string('private_slide', 'voiceemail'));
        $this->xml->createLine();
    ...
    */     
    function createVBFeaturesPanel() {
        ...
        $this->xml->addInputElement($parameters);
        $this->xml->addSimpleLineElement("label", get_string('short_message', 'voiceemail'));
        ...
        $this->xml->addInputElement($parameters);
        $this->xml->addSimpleLineElement("label", get_string('chrono_order', 'voiceemail'));
        ...
        $this->xml->addInputElement($parameters);
        $this->xml->addSimpleLineElement("label", get_string('show_forward', 'voiceemail'));
        ...
        $this->xml->addInputElement($parameters);
        $this->xml->addSimpleLineElement("label", get_string('settings_show_reply', 'voiceemail'));
        ...
        if(isGradebookAvailable()){
          ...
          if(isset ($this->currentObject)){
            $activities = $DB->get_records("voiceemail", array("rid" => $this->currentObject->getRid()));
            if(empty($activities)){
             $parameters['disabled'] = "true";
            }
          }else{
          ...
          $this->xml->addInputElement($parameters);
          $this->xml->addSimpleLineElement("label", get_string('grade_settings', 'voiceemail'));
    ...
    */   
    function getListVoiceTools() {
        ...
        for ($i = 0; $i < count($ressources); $i++) 
        {
            ...
            $xmlResource->setTooltipAvailability(get_string("tooltipVT_" . $preview . "_student", 'voiceemail'));
