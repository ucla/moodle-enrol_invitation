...
<?php
    ...
    $result=mysql_query("select enrol,b.name,b.shortname,count(*) as cnt from {$CFG->prefix}role_assignments a left join {$CFG->prefix}role b on(a.roleid=b.id) group by enrol,roleid", $db_moodle) 
        or die("Unable to mdl_role_assignments: " . mysql_error());
//  $coursers = mysql_query("select id,idnumber from {$CFG->prefix}course where idnumber like '$term-$srs%'", $db_moodle) or die("Unable to get     course IDs from Moodle: " . mysql_error());
    ...
    echo "<table>\n";
    echo "<tr><td><b>Source</b></td><td><b>Role Name</b></td><td><b>Role Shortname</b></td><td><b>Count</b></td></tr>\n";
...
<?php
    ...
    if ($_POST['radio'] == "all") {
        ...
        $log_query = "select from_unixtime(time) as time, a.userid, b.firstname,b.lastname,ip,info
        from {$CFG->prefix}log a
        left join {$CFG->prefix}user b on (a.userid=b.id)
        ...
        $log_query = "select 
            ...
            ip, 
            info,
            CONCAT('<a href=\"{$CFG->wwwroot}/course/view.php?id=', a.course, '\">', c.shortname, '</a>') as url
        from {$CFG->prefix}log a
        left join {$CFG->prefix}user b on (a.userid = b.id)
        left join {$CFG->prefix}course c on (a.course = c.id)
    ...
    // END UCLA SSC MODIFICATION #1095
    echo "<table>\n";
    $cols = 0;
...
<?php
    ...
    $db_moodle = mysql_connect($CFG->dbhost,$CFG->dbuser,$CFG->dbpass)
     ...
     mysql_select_db($CFG->dbname, $db_moodle) 
        or die("Unable to select DB {$CFG->dbname}");
     $result=mysql_query("select from_unixtime(time) as time,b.firstname,b.lastname,ip,a.url,info
        from {$CFG->prefix}log a 
        left join {$CFG->prefix}user b on(a.userid=b.id)
        where from_unixtime(time)  >= DATE_SUB(CURDATE(), INTERVAL 1 DAY) and action='login'
        order by a.id desc", $db_moodle)
        or die(mysql_error());
...
// save for later when figure out how sql query should look    <input type="radio" name="radio" value="unique" CHECKED>Unique Logins
    ...
    if ($whereclause=="login") {
        ...
        echo "<h3>Count of Moodle Logins for Last $days Days</h3>\n";
    } else {
        $whereclause = "";
        echo "<h3>Count of Moodle Log Entries for Last $days Days</h3>\n";
    }    
    $db_moodle = mysql_connect($CFG->dbhost,$CFG->dbuser,$CFG->dbpass)
        or die("Unable to connect to Moodle DB server {$CFG->dbhost}");
     mysql_select_db($CFG->dbname, $db_moodle) 
    ...
    $days--;  # decrement days by 1 to get query to work
     $result=mysql_query("select from_unixtime(time,'%Y-%m-%d') as day,count(*) as cnt 
        from {$CFG->prefix}log a 
...
<?php
    ...
    $db_moodle = mysql_connect($CFG->dbhost,$CFG->dbuser,$CFG->dbpass)
     ...
     mysql_select_db($CFG->dbname, $db_moodle) 
        or die("Unable to select DB {$CFG->dbname}");
     $result=mysql_query("select from_unixtime(time,'%Y-%m-%d') as day,c.shortname as course,count(*) as cnt 
        from {$CFG->prefix}log a 
        left join {$CFG->prefix}course c on(a.course=c.id) 
...
<?php
    ...
    $db_moodle = mysql_connect($CFG->dbhost,$CFG->dbuser,$CFG->dbpass)
     ...
     mysql_select_db($CFG->dbname, $db_moodle) 
        or die("Unable to select DB {$CFG->dbname}");
     $result=mysql_query("select from_unixtime(time,'%Y-%m-%d') as day,c.shortname as course,b.firstname,b.lastname,count(*) as cnt 
        from {$CFG->prefix}log a 
        left join {$CFG->prefix}user b on(a.userid=b.id) 
        left join {$CFG->prefix}course c on(a.course=c.id) 
        where from_unixtime(time)  >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) 
...
////////////////////////////////////////////////////////////////////
$title="Comparison of ucla_request_classes & ucla_request_crosslisted with classes table.";
if (empty($_POST['console'])) { 
    printhead();
?>
    <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
...
<?php    
    ...
    mysql_connect($CFG->classrequestor_dbhost, $CFG->classrequestor_dbuser, $CFG->classrequestor_dbpass) or die("Unable to connect to DB server $CFG->classrequestor_dbhost as user $CFG->classrequestor_dbuser");
    mysql_select_db($CFG->classrequestor_dbname) or die("Unable to select DB $CFG->classrequestor_dbname");

    echo "<h3>$title</h3>\n";
    echo "NULL in added_at column means the classes table does not have that record.<br>\n";

     $result=mysql_query("select d.tbl,d.term,d.srs,d.course,d.action,d.status,b.dl_inst_only,b.added_at 
        from ((select 'ucla_request_classes' as tbl,a.term,a.srs,a.course,a.action,a.status from ucla_request_classes a )
        union (select 'ucla_request_crosslist' as tbl,c.term,c.aliassrs,'course','action','status'
        from ucla_request_crosslist c)) as d
        left join classes b 
        using(term,srs) 
        order by d.term,d.course")
        or die(mysql_error());
...
<?php
    ...
    if (!empty($firstname) and !empty($lastname)) {
        $whereclause = "WHERE firstname LIKE \"$searchstart$firstname$searchend\" AND lastname LIKE \"$searchstart$lastname$seachend\" AND deleted=0"; 
    ...
    $db_moodle = mysql_connect($CFG->dbhost,$CFG->dbuser,$CFG->dbpass)
     ...
     mysql_select_db($CFG->dbname, $db_moodle) 
        or die("Unable to select DB {$CFG->dbname}");
     $result=mysql_query("select id,auth,username,firstname,lastname,idnumber,email,from_unixtime(lastaccess) as last_access,lastip
        from {$CFG->prefix}user a 
        $whereclause
        order by a.lastname,a.firstname", $db_moodle)
        or die(mysql_error());
        $num_rows = mysql_num_rows($result);
        if ($num_rows === 1) {
...
<?php
	...
	if($itemfile=='forumposts'){
		$result=mysql_query("SELECT CONCAT('<a target=\"_blank\" href=\"{$CFG->wwwroot}/course/edit.php?id=',c.id,'\">',c.id,'</a>') as ID, 
							CONCAT('<a target=\"_blank\" href=\"{$CFG->wwwroot}/course/view.php?id=',c.id,'\">',c.shortname,'</a>') as Course ,count(*) as Posts, c.fullname as Full_Name
							FROM mdl_course c 
	...
	$result=mysql_query("SELECT COUNT(l.id) count, CONCAT('<a target=\"_blank\" href=\"{$CFG->wwwroot}/course/view.php?id=',c.id,'\">',c.shortname,'</a>') as coursename
        FROM {$CFG->prefix}$itemfile l
        		INNER JOIN {$CFG->prefix}course c on l.course = c.id
        WHERE c.idnumber like '$term%'        
        GROUP BY left(c.idnumber,3), course
        ORDER BY left(c.idnumber,3), count DESC", $db_moodle)
        or die(mysql_error());
...
?>
    <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    <input type="submit" name="console" value="<?php echo $title; ?>">
    TERM: <input type="textfield" name="term" value="<?php echo $CFG->currentterm; ?>">
...
?>
    <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
        <input type="submit" name="console" value="<?php echo $title; ?>">
        TERM: <input type="textfield" name="term">
        SRS: <input type="textfield" name="srs">
    </form>
<?php
} elseif ($_POST['console'] == "$title") {  # tie-in to link from name lookup
...
if (empty($_POST['console'])) { 
    ...
    <input type="submit" name="console" value="<? echo $title; ?>">
...
if (empty($_POST['console'])) {
    ...
    $result=mysql_query("select 
                        CONCAT('<a target = \"blank\" href=\"{$CFG->wwwroot}/course/view/', shortname ,'\">',shortname,'</a>') as shortname,idnumber 
...
if (empty($_POST['console'])) {
    ...
    $result=mysql_query("
    SELECT m.duedate, CONCAT('<a href=\"{$CFG->wwwroot}/course/view/',c.shortname,'\">',c.shortname,'</a>') as class, c.fullname, m.modtype, m.name
    FROM ((
            SELECT 'quiz' AS modtype, course, name, from_unixtime( timeclose ) AS duedate
            FROM `mdl_quiz`
            WHERE timeclose
            BETWEEN  {$timefrom}
            AND {$timefrom} + {$days}*24*3600
        ) UNION (
            SELECT 'assignment' AS modtype, course, name, from_unixtime( timedue , '%m-%d-%y %H:%i %a') AS duedate
            FROM mdl_assignment
            WHERE timedue
            BETWEEN {$timefrom}
            AND {$timefrom} + {$days}*24*3600
...
///////////////////////////////////////////////////////////////////////
// START SSC MODIFICATION #1085 : Show courses that will send things //
//  to MyUCLA Gradebook.                                             //
///////////////////////////////////////////////////////////////////////
$title = "Courses with Grades that can be sent over to MyUCLA Gradebook";
...
?>
    ...
    Select a TERM: 
    <select name="term">
    <option value="ALL TERMS">All Term Summary</option>
<?php
    $terms = get_records_sql("
        SELECT DISTINCT
            LEFT(c.idnumber, 3) AS term
        FROM mdl_course c
...
?>
    ...
    <input type="checkbox" name="items">Show each courses' grade items as well</input>
    </form>
<?php
    ...
    $sql = "
        SELECT 
            CONCAT(gi.id, '-', gg.id, '-', c.id) AS rid,
            LEFT(c.idnumber, 3) AS term,
            CONCAT('<a href=\"{$CFG->wwwroot}/course/view.php?id=', c.id, '\">', 
                c.shortname, '</a>') AS url,
        ";

    if ($groupby) {
        $groupsql = 'gi.id';
        $keyword = 'gradable item';
        $sql .= "
            gi.itemname AS 'Graded Item Name', 
            gi.itemtype AS 'Grade Item Type', 
            gi.itemmodule AS 'Grade Item Module',
        ";
    }

    $where = '';
    if ($_POST['term'] != 'ALL TERMS') {
        $sql .= "
                COUNT(*) as count_grades_sent
            FROM mdl_course c 
            INNER JOIN mdl_grade_items gi ON gi.courseid = c.id
            INNER JOIN mdl_grade_grades gg ON gi.id = gg.itemid
            WHERE LEFT(c.idnumber, 3) = '" . $_POST['term'] . "'
            GROUP BY $groupsql
            ORDER BY LEFT(c.idnumber, 3) 
        ";
    } else {
        $sql = "
        SELECT 
        ...
        FROM mdl_course c
        WHERE c.id IN (
            SELECT courseid FROM mdl_grade_items
                WHERE id IN (
                    SELECT itemid FROM mdl_grade_grades
                )
            )
        GROUP BY LEFT(c.idnumber, 3)
        ";
    }
    ...
    echo "There is/are " . count($records) . " $keyword(s).<br>";

    $first = reset($records);
    $firstarr = get_object_vars($first);
    $afields = array_keys($firstarr);
    ...
    foreach ($records as $record) {
        ...
        foreach ($record as $fiel => $data) {                
            if (!in_array($fiel, $fields)) {
                continue;
            }
...
<?php 
    ...
    $result=mysql_query("SELECT c.shortname AS c_shortname, m.name AS m_name, count( * ) AS cnt
                         FROM mdl_course c
                         JOIN mdl_course_modules cm ON c.id = cm.course
                         JOIN mdl_modules m ON cm.module = m.id
                         GROUP BY c.id, m.id") or die(mysql_error());
    echo "<table>\n";
    echo "<tr>\n";
    echo "<th>Course</th>\n";
