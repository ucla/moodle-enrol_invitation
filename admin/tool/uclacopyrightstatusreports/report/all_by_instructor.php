...
$filter_instr = optional_param('filter_instructor', '', PARAM_TEXT);
$filter_array = array('term'=>$filter_term, 'uid'=>substr($filter_instr, 1));
...
// Heading
echo $OUTPUT->heading(get_string('all_by_instructor', 'tool_uclacopyrightstatusreports'), 2, 'headingblock');
...
if (empty($list)) {
    ...
    foreach($list as $term=>$list1){
        ...
        $table->head = array(get_string('instructor_name', 'tool_uclacopyrightstatusreports'), get_string('class_name', 'tool_uclacopyrightstatusreports'), 
        ...
        $totalcell = new html_table_cell();
        $totalcell->colspan = 2;
        $totalcell->text = html_writer::tag('span', html_writer::tag('strong', get_string('total_files', 'tool_uclacopyrightstatusreports')), array('class'=>'grouping-text'));
        $row_term[] = $totalcell;
        $row_term[] = $total;
        $row_term[] = $tbd_term;
        $row_term[] = $iown_term;
