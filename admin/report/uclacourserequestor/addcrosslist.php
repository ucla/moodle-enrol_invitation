...
if (empty($termcleaned)) {
    $term = $CFG->classrequestor_selected_term;
} else {
    $term = $termcleaned;    
}
...
echo '<select name="hostsrs" >';

$sql = "SELECT      srs,
                    course 
        FROM        ".$CFG->prefix."ucla_request_classes 
        WHERE       term='$term' AND 
                    action LIKE '%uild' AND 
                    (status = 'processing' OR 
                     status = 'pending') 
        ORDER BY    course";
$crs = $DB->get_records_sql($sql);
if (empty($crs)) {
    echo "<option value=''>".  get_string('queueempty', 'report_uclacourserequestor')."</option>";    
} else {
    foreach ($crs as $rows) {
        $srs = trim($rows->srs);
        $course = trim($rows->course);
        $existingcourse[$srs] = 1;
        echo "<option value='$srs'>$course ($srs)</option>";
    }    
}

echo '</select></label><br />';
echo '(' . get_string('crosslistnotice', 'report_uclacourserequestor') . ')<br />';
