...
section_class.prototype.init_section = function(id, group, config, isDraggable) {
    ...
    // START UCLA Modification: CCLE-2229/2904 - Public/Private for Moodle 2.x
    this.publicprivateButton = null;
    // END UCLA Modification: CCLE-2229/2904 - Public/Private for Moodle 2.x
...
resource_class.prototype.init_buttons = function() {
    ...
    // START UCLA Modification: CCLE-2229 - Public/Private for Moodle 2.x
    this.publicprivate = -1;
    // END UCLA Modification: CCLE-2229 - Public/Private for Moodle 2.x    
    ...
    if (deletePresent) {
        var button = main.mk_button('a', main.portal.icons['delete'], main.portal.strings['delete'], [['class', 'editing_delete']], [['class', 'iconsmall']]);
    ...
    if (hideshow) {
        if (this.hidden) {
            var button = main.mk_button('a', main.portal.icons['show'], main.portal.strings['show'], [['class', 'editing_hide']], [['class', 'iconsmall']]);
        } else {
            var button = main.mk_button('a', main.portal.icons['hide'], main.portal.strings['hide'], [['class', 'editing_hide']], [['class', 'iconsmall']]);
        }
    ...
    if (this.publicprivate == 0) {
        var button = main.mk_button('a', main.portal.icons['publicprivate_public'], main.portal.strings['publicprivate_makeprivate'], [['class', 'editing_makeprivate']]);
    } else if(this.publicprivate > 0) {
        var button = main.mk_button('a', main.portal.icons['publicprivate_private'], main.portal.strings['publicprivate_makepublic'], [['class', 'editing_makepublic']]);
    }
    YAHOO.util.Event.addListener(button, 'click', this.toggle_publicprivate, this, true);
    commandContainer.appendChild(button);
    this.publicprivateButton = button;
     // END UCLA Modification: CCLE-2229/2904 - Public/Private for Moodle 2.x
...
// START UCLA Modification: CCLE-2229/2904/3058 - Public/Private for Moodle 2.x     
resource_class.prototype.toggle_publicprivate = function(target, e, superficial, force) {
    ...
    if (this.publicprivate == 0) {
        ...
        this.publicprivateButton.childNodes[0].src = this.publicprivateButton.childNodes[0].src.replace(/public/i, 'private');
        this.publicprivateButton.childNodes[0].alt = this.publicprivateButton.childNodes[0].alt.replace(strpublic, strprivate);
        this.publicprivateButton.title = this.publicprivateButton.title.replace(strpublic, strprivate);
        this.publicprivate = 1;
        if (!superficial) {
        ...
        this.publicprivateButton.childNodes[0].src = this.publicprivateButton.childNodes[0].src.replace(/private/i, 'public');
        this.publicprivateButton.childNodes[0].alt = this.publicprivateButton.childNodes[0].alt.replace(strprivate, strpublic);
        this.publicprivateButton.title = this.publicprivateButton.title.replace(strprivate, strpublic);
        this.publicprivate = 0;
...
// END UCLA Modification: CCLE-2229/2904/3058 - Public/Private for Moodle 2.x     
...
resource_class.prototype.onDragOver = function(e, ids) {
    ...
    if (target != this && (target.is == 'resource' || target.is == 'activity')) {
        ...
        YAHOO.util.Dom.setStyle(target.getEl().id, 'border-top', '2px dashed orange');
