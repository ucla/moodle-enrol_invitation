...
/**
 * Report to get all collab sites over the specified file size limit
 * and a breakdown of number of files and the total size 
 * for each type of mimetype of a corresponding collab site
 *
 * @package    report
...
class large_collab_sites extends uclastats_base {
    ...
    /**
     * Query to get all collab sites over the specified file size limit
     * and a breakdown of number of files and the total size 
     * for each type of mimetype of a corresponding collab site
     *
     * @param array $params
    ...
    public function query($params) {
        ...
        $sql_body =  " FROM {course} c
                       LEFT JOIN {ucla_siteindicator} AS si ON (
                           c.id = si.courseid
                       )
        ...
        //query to locate all large collab sites
        ...
        //query to get all files belonging to large collab sites
        $sql2 = "SELECT DISTINCT f.id,f.mimetype, f.filesize"
                . $sql_body .
                " AND c.id = :collab_id";

        //update query result with file information for corresponding collaboration sites
        foreach ($query_result as &$course) {
            $params["collab_id"] = $course->id;
