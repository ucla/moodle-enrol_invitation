...
class admin_setting_confightml extends admin_setting {
     ...
     * for settings that in config, or 'myplugin/mysetting' for ones 
     * in config_plugins.
     *
    ...
    public function write_setting($data) {
        ...
        if (!empty($_POST['s__' . $this->name])) {
            ...
            foreach ($_POST['ids'] as $k => $id) {
                ...
                $data->elements[] = $this->_block->create_element($id, $name, 
                    $url, $icon, $canHide, $visible);
            }

            //links
            $data->links = array();
            if (isset($_POST['linkNames'])) { 
                // means: if instance config. we don't have links 
                // in global config
