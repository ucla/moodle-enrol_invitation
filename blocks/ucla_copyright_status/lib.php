...
function calculate_copyright_status_statistics($filelist){
	...
	foreach($filelist as $record){
		$sum_array[$record->license] = isset($sum_array[$record->license])?$sum_array[$record->license]:0;
		$sum_array[$record->license]++; // calculate each type total
		$total++;
	}
	...
	foreach ($data as $key => $value){
		...
		if (isset($id)){
			...
			foreach ($id_array_with_same_contenthash as $fid => $other){
				$params = array('id'=>$fid, 'license'=>$value);
...
function display_copyright_status_contents($courseid, $filter){
	...
	foreach($license_options as $k=>$v){
		if ($k != 'all'){
			echo html_writer::tag('li', $v.':('.$stat_array[$k].'/'.$stat_array['total'].', '.number_format($stat_array[$k]*100/$stat_array['total'],0,'','').'%)');
		}
	}
    ...
    foreach ($course_copyright_status_list as $record) {
		$select_copyright = html_writer::select($license_options, 'filecopyright_'.$record->id, $record->license, false);
		$t->data[] = array($record->filename, $record->timemodified, $record->author, $select_copyright); 
	}
