    ...
    if ($showsection) {
        ...
        if ($editing && $has_capability_update) {
            ...
            if ($section != 0) {
                ...
                $safe_controls[] = html_writer::link($moodle_url, $innards, $link_options);

                // Add delete?

                $right_side .= html_writer::tag(
                                'span', implode(' ', $safe_controls), array('class' => 'safecontrol')
                );
        ...
        if (!$has_capability_viewhidden and !$thissection->visible) {
            ...
            if ($section == 0) {
                ...
                if ($editing && $has_capability_update) {
                    ...
                    $center_content .= html_writer::tag('span', html_writer::link($moodle_url, $innards, $link_options), array('class' => 'editbutton'));
                }
